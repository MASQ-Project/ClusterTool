...
def test_commands():
    ...
    assert COMMANDS['start'].info == "starts MASQNode on"
    assert COMMANDS['tail'].name == "tail"
    assert COMMANDS['tail'].info == "opens terminal(s) to tail -f MASQNode logs on"
    ...
    assert COMMANDS['stop'].info == "reverts and stops MASQNode on"
